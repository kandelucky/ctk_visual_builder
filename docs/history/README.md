...
|---------|-----------|------------|
| **v1.3.0** | [![v1.3.0](v1.3.0.png)](v1.3.0.png) | **Cross-doc reparent dialog + per-project component library.** Moving a widget across windows asks how to handle its local-variable bindings; same flow on Ctrl+C/V/X with same-name reuse on round-trips. Components (renamed from prefabs) live under `<project>/components/` with save / export / import / preview dialogs. Canvas right-click gains an `Add Widget ▶` cascade. Per-document Snap & Alignment toggles. |
