...
>
> Earlier in v1.10: **Exported CTkButton auto-syncs icon tint** with state changes — `btn.configure(state="disabled")` now swaps the tinted icon variant automatically (v1.10.6). **Every exported CTkButton routes through `CircleButton`** so icon-only buttons no longer regress to the grown-rectangle bug (v1.10.5). **Exported CTkScrollableFrame keeps its scrollbar AND flex-distributes grow children to the actual viewport** (v1.10.4). **Drag snap-back fires the same on all four doc edges** (v1.10.3). **Horizontal / Vertical layouts flex-shrink** their children to a content-min floor instead of clipping the latest one off the edge (v1.10.2). **Generated `.py` is ~21% smaller** (v1.10.0 / v1.10.1).
>
