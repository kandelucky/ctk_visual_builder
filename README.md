...
> **v1.9.10** — **Full-circle CTkButton with text** renders cleanly — the upstream `_create_grid` reservation that pushed the outer Frame to grow on max-radius buttons with text is patched via an inlined `CircleButton` subclass. Calculator-style designs (60×60 + radius=30 + "C", "÷", …) now place neighbours without overlap in workspace, F5 preview, and exported `.py`.
>
...
- **Visual scripting (event handlers + behavior fields)** — clickable widgets gain an **Events** group in the Properties panel: pick a button, click `[+]` next to `<Button-1>`, the exporter generates a stub method in `<project>/assets/scripts/<page>/<window>.py` and binds it. Multi-method bindings (run several callbacks on one click) supported with reorder + unbind. The behavior class also exposes typed slots — declare `submit_btn: ref[CTkButton]` and the Inspector grows a `[Pick…]` button to assign a canvas widget; `setup(self)` runs after slots are wired so user code can lean on `self.submit_btn` directly. Editor preference (Settings → Editor) routes `Open in editor` / F7 / double-click into VS Code, Notepad++, or IDLE — auto-detect defeats Git Bash `code` shadowing on Windows.
