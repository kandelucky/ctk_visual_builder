...
> **v1.13.6** — **Color as a first-class variable type.** Variables window now offers `Color` as a fifth type alongside `String / Integer / Float / Boolean`. Color rows show a swatch in the panel + edit via picker (no more typing `#hex` into a String row). Color properties (`fg_color`, `text_color`, `border_color`, …) only see `color` + `str` variables in the bind menu. Default changes repaint bound widgets live on the canvas.
