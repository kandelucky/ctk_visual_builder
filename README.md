...
> **v1.15** — **Content-first reorder.** Every widget descriptor reorders its Properties panel groups to Content → Layout → Visual → Behavior, so the rows you change every time (Text, Icon, Values) sit at the top and the developer-wiring rows (Events, Object Reference) settle at the bottom. The palette catalog collapses from 6 groups to 5 — Display / Controls / Containers / Layouts / Indicators — with Buttons + Selection + Input merging into a single Controls block and Progress widgets splitting into a new Indicators group at the end.
> **v1.13.6 → v1.14.6 — Variables, Label parity, Properties polish:** Color joins as a fifth variable type with swatch picker + live repaint; double-click a `var:`-bound property row to jump straight to the Variables window. Label reaches feature parity with Button — Icon group, Rectangle, Alignment (anchor positions the whole text+icon block), Interaction (cursor / takefocus / disabled visuals), 16 bind events split into Default + Advanced sub-groups. Property rows gain hover tooltips with descriptions + warnings sourced from `property_help.py` and `EventEntry.description`. Window panel: per-window Object Reference toggle moves up next to window-side properties, separated from Local Variables / Object References by a darker spacer. Internal: `properties_panel_v2` package renames back to `properties_panel`.
> **v1.11.1 → v1.13.5 — Object References + polish round:** Object References replace Behavior Fields with a first-class reference system (per-widget +/× toggle in Properties; F11 → Object References tab; typed `self.<name>: <Type>` slots in exported behavior classes; legacy `behavior_field_values` migrate silently). Plus: startup screen with logo + version, persisted window / panel collapse state across restarts, layout-independent tool shortcuts (Q / W / E via hardware keycode), pill-style tab switchers in palette and right panels, Best-Fit / Wrap interaction fixes on Label, multi-page save no longer overwrites page-file `name`.
