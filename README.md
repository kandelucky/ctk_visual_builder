...
> **v1.11.0** — **Object References: replace Behavior Fields with a first-class reference system.** The old Behavior Fields group (which showed identical content on every widget's panel) is gone. In its place: a single-row toggle in each widget's Properties panel — click **+** to create a local object reference for that widget, **×** to remove it. Window / Dialog panels get their own global toggle for cross-window references. All references are managed in **F11 → Object References** (new third tab alongside Global and Local variables): rename, retarget, delete — scoped like variables: global refs live on the project, local refs live on a single window. On export, local refs emit as typed `self.<name>: <Type>` slots in the window's behavior class; global refs resolve to the correct class symbol across documents. Legacy `behavior_field_values` entries in older `.ctkproj` files migrate silently on first open.
